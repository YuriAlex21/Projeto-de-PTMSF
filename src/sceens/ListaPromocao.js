...
export default function ListaPromocao({navigation}) {
    ...
    {
        try {
            const response = await fetch(`${config.urlRoot}listaPromocao`);
    ...
    return (
        <View style={[css.container, css.containerTop]}>
            ...
            {/* <Text>{dados}</Text> setIdItem(item.id) {sendForm(),setIdItem(item.id)}*/}
            ...
            {isLoading ? <ActivityIndicator/> : (
                ...
                renderItem={({ item }) => (
                    ...
                    <Text  style={css.lista__PromoText}>Local: {item.local}</Text>
                    <Text  style={css.lista__PromoText}>Valor: R${item.valor}</Text>
                    <TouchableOpacity style={css.Add__button} > 
                        <Text style={css.login__buttonText} onPress={()=>{sendForm(item.id)}}>Comprar</Text>
                    </TouchableOpacity>
                    </View>
                )}
                />
